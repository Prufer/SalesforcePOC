...
def complete_consultation_booking(page, salesforce_urls, consultation_forms, is_international=True):
    ...
    profile_url = salesforce_urls["consultation_international_booking_url" if is_international else "consultation_domestic_booking_url"]
    ...
    fill_personal_details(page, consultation_forms.selectors, is_international=is_international)
...
def test_first_user_booking(salesforce_urls, page, shared_context):
    ...
    try:
        consultation_international = ConsultationForms(page)
        selected_slot, attempts = complete_consultation_booking(page, salesforce_urls, consultation_international, is_international=True)
        shared_context["selected_slot"] = selected_slot
        shared_context["attempts"] = attempts

        # Confirm booking and validate thank you message
        consultation_international.confirm_booking()
        consultation_international.confirm_final_message()
        consultation_international.click_finish_button()
        consultation_international.validate_thank_you_message()
...
def test_second_user_validate_time_slot(salesforce_urls, page, shared_context):
    ...
    try:
        ...
        # Fill personal and academic details
        fill_personal_details(page, consultation_international.selectors, is_international=True)
        fill_academic_details(page, consultation_international.selectors)

        # Proceed to appointment selection and navigate to the same date page
        consultation_international.proceed_to_appointment_selection()
        consultation_international.select_appointment_type()
        navigate_to_date_page(page, consultation_international.selectors, attempts)

        # Validate that the previously selected time slot is unavailable
        is_unavailable = is_time_slot_unavailable(page, consultation_international.selectors, selected_slot)
